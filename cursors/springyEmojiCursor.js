function springyEmojiCursor(options) {
  ...
  function updateParticles() {
      ...
      for (let i = 1; i < nDots; i++) {
          ...
          let resist = new vec(-particles[i].velocity.x * RESISTANCE, -particles[i].velocity.y * RESISTANCE);

          let accel = new vec((spring.X + resist.X) / MASS, (spring.Y + resist.Y) / MASS + GRAVITY);

          particles[i].velocity.x += DELTAT * accel.X;
          particles[i].velocity.y += DELTAT * accel.Y;

          if (Math.abs(particles[i].velocity.x) < STOPVEL && Math.abs(particles[i].velocity.y) < STOPVEL && Math.abs(accel.X) < STOPACC && Math.abs(accel.Y) < STOPACC) {
  ...
  function Particle(canvasItem) {
      ...
      this.draw = function(context) {
          context.drawImage(this.canv, this.position.x - this.canv.width / 2, this.position.y - this.canv.height / 2, this.canv.width, this.canv.height);
      };
  }
...
window.springyEmojiCursor = springyEmojiCursor;
